...
from cascade_planner.legacy.harness_runtime.analogical_reaction_templates import (
...
from cascade_planner.legacy.harness_runtime.hypothetical_retrosynthesis_report import (
    compile_hypothesis_only_retrosynthesis_report,
)
from cascade_planner.legacy.harness_runtime.hypothesis_execution_report import (
...
from cascade_planner.research.open_research_experience import audit_local_pdf_proxy_fallback
...
from cascade_planner.legacy.harness_runtime.schemas import TargetInput, ToolCallRecord
from cascade_planner.legacy.harness_runtime.target_side_strategy import (
    build_target_side_disconnection_hypotheses,
)
...
from scripts.legacy.resume_agentic_blackboard import (
    _compact_cli_result,
...
_SOURCE_FIXTURES = Path(__file__).parents[1] / "fixtures"
...
_RENDERED_PAGE_FIXTURE = (Path(__file__).parents[1] / "fixtures" / "source_page.ppm").resolve()
_PDF_FIXTURE = (Path(__file__).parents[1] / "fixtures" / "source_evidence_stub.pdf").resolve()
...
class AgenticBlackboardControllerTest(unittest.TestCase):
    ...
    def test_agentic_cli_max_chem_enzy_runs_alias_disables_guided_when_unset(self):
        from scripts.legacy.run_codex_entry_agentic_blackboard import _budget_from_args
    ...
    def test_codex_action_planner_does_not_retry_timeout_worker(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.dict(
            ...
            "cascade_planner.legacy.harness_runtime.codex_action_planner.run_codex_worker",
    ...
    def test_guided_chemenzy_large_atom_jump_overrides_backend_solved_when_no_route_accepted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "cascade_planner.legacy.harness_runtime.tools._execute_chemenzy_request",
                return_value=backend_result,
            ), patch(
                "cascade_planner.legacy.harness_runtime.tools.verify_chemenzy_raw_routes",
    ...
    def test_guided_chemenzy_timeout_is_blackboard_feedback_not_tool_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("cascade_planner.legacy.harness_runtime.tools._execute_chemenzy_request", return_value=backend_result):
                output = run_guided_chemenzy_rerun(state, payload)
    ...
    def test_visual_direct_api_failure_is_auditable_tool_result(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "cascade_planner.legacy.harness_runtime.tools._visual_chain_image_paths",
                return_value=[image],
            ), patch(
    ...
    def test_visual_partial_candidate_is_salvaged_when_packaging_missing_result_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                ...
                "cascade_planner.legacy.harness_runtime.tools._attach_process_evidence_rows_to_visual_result",
                side_effect=FileNotFoundError(str(out / "visual_literature_chain_extraction_result.json")),
            ):
                record = execute_local_tool(
                    "extract_visual_literature_chain",
