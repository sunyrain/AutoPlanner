...
def test_capability_probe_uses_selected_interpreter_and_bounded_child_environment(
    ...
    def fake_run(command: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        assert kwargs["env"]["PYTHONPATH"] == str(Path(__file__).resolve().parents[2])  # type: ignore[index]
...
def test_tools_runtime_preflight_passes_actual_request_selection() -> None:
    with patch(
        "cascade_planner.legacy.harness_runtime.tools.diagnose_chem_enzy_runtime",
...
def test_runtime_unavailable_chemenzy_result_is_persisted_at_referenced_output(
    ...
    with patch(
        "cascade_planner.legacy.harness_runtime.tools._chem_enzy_runtime_preflight",
...
def test_timed_out_chemenzy_result_is_persisted_at_referenced_output(
    ...
    with (
        patch(
            "cascade_planner.legacy.harness_runtime.tools._chem_enzy_python_bin",
            return_value=Path("python"),
        ),
        patch("cascade_planner.legacy.harness_runtime.tools.subprocess.Popen", return_value=process),
        patch("cascade_planner.legacy.harness_runtime.tools._terminate_process_group"),
        patch(
            "cascade_planner.legacy.harness_runtime.tools._runtime_preflight_for_python",
            return_value=runtime_preflight,
        ),
    ):
        result = _execute_chemenzy_request(
