...
from cascade_planner.legacy.harness_runtime.codex_plan import (
    _workflow_plan_normalization_audit,
    deterministic_workflow_plan,
)
from cascade_planner.legacy.harness_runtime.preflight import run_preflight
from cascade_planner.legacy.harness_runtime.runner import (
...
from cascade_planner.research.route_failure_feedback import compile_route_failure_feedback
...
from cascade_planner.research.open_research_retrieval import prefetch_open_research_evidence
from cascade_planner.research.source_detail_chain_builder import (
    resolve_curator_records_to_source_detail_steps,
)
from cascade_planner.research.source_detail_resolution import source_detail_curator_records_path
from cascade_planner.providers.stock import (
    canonicalize_stock_snapshot,
    stock_snapshot_sha256,
)
from scripts.legacy.run_codex_entry_agentic_blackboard import (
    _codex_action_planner_env_overrides,
    _codex_agent_team_runtime_args,
    _trusted_stock_snapshots_from_args,
)
from scripts.legacy.run_codex_entry_controller import _resolve_cli_targets
...
SOURCE_EVIDENCE_MANIFEST_FIXTURE = (
    Path(__file__).parents[1] / "fixtures" / "source_evidence_manifest.json"
).resolve()
TRUSTED_LITERATURE_REGISTRY_FIXTURE = (
    Path(__file__).parents[1] / "fixtures" / "trusted_literature_step_registry.json"
).resolve()
...
class CodexEntryHarnessContractTest(unittest.TestCase):
    ...
    def test_cli_accepts_positional_smiles_and_creates_batch_run_dirs(self):
        with tempfile.TemporaryDirectory() as tmp:
            proc = subprocess.run(
                [
                    sys.executable,
                    "scripts/legacy/run_codex_entry_controller.py",
                    "not_a_smiles",
                    "still_not_a_smiles",
                    "--output-dir",
                    tmp,
    ...
    def test_structure_resolution_uses_target_identity_without_visual_call(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("cascade_planner.legacy.harness_runtime.tools.run_visual_literature_chain_agent") as visual_mock:
    ...
    def test_open_structure_research_internal_timeout_rejects_launcher_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("cascade_planner.legacy.harness_runtime.tools.subprocess.run", side_effect=fake_run):
    ...
    def test_chemenzy_tool_passes_condition_prediction_flags(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("cascade_planner.legacy.harness_runtime.tools._chem_enzy_python_bin", return_value=Path("/usr/bin/python3")):
                with patch("cascade_planner.legacy.harness_runtime.tools.subprocess.run", side_effect=fake_run):
