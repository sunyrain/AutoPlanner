...
from scripts.legacy.evaluate_agentic_run import compare_reports, evaluate_run, main
    ...
    result = subprocess.run(
        [
            ...
            (
                ...
                "import scripts.legacy.evaluate_agentic_run as module; "
                "from pathlib import Path; "
...
def test_evaluator_direct_script_cli_loads_shared_stdlib_helper(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(repo_root / "scripts" / "legacy" / "evaluate_agentic_run.py"),
            "--help",
        ],
        cwd=tmp_path,
        check=False,
