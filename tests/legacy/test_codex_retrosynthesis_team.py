...
import cascade_planner.legacy.orchestration_runtime.codex_retrosynthesis as codex_retrosynthesis_module
...
from cascade_planner.legacy.harness_runtime.codex_edge_verification import (
...
from cascade_planner.legacy.orchestration_runtime.admitted_hyperedges import (
    canonical_graph_step_id,
    canonical_graph_step_signature,
)
from cascade_planner.legacy.application_runtime.frontier_ledger import exact_edge_signature
from cascade_planner.legacy.routes_runtime.admission_receipts import (
    make_chemenzy_admission_material,
)
from cascade_planner.routes.admission import audit_retrosynthetic_candidate
from cascade_planner.legacy.routes_runtime.graph import select_route_consensus_frontier
...
def test_global_campaign_schema_requires_operational_condition_predictions() -> None:
    task = WorkerTask(
        task_id="director",
        case_id="campaign-1",
    ...
    step = (
        properties["multi_step_skeletons"]["items"]["properties"]["steps"]["items"]
    )
    condition_predictions = step["properties"]["condition_predictions"]
    condition = condition_predictions["items"]
    source_task = properties["source_plan"]["items"]
    frontier = properties["frontier_priorities"]["items"]

    assert "condition_predictions" in step["required"]
    assert condition_predictions["minItems"] == 1
    assert condition_predictions["maxItems"] == 2
    assert condition["properties"]["authority_scope"]["enum"] == [
        "model_predicted_condition"
    ]
    assert condition["properties"]["not_reaction_proof"]["enum"] == [True]
    assert "source_refs" in source_task["required"]
    assert {
        "target_smiles",
        "provider_preferences",
        "retron_hints",
        "route_family_ids",
    } <= set(frontier["required"])
...
def test_proof_reconciliation_rehydrates_policy_bound_benchmark_provider(
    ...
    from cascade_planner.legacy.application_runtime.frontier_scheduler import PersistentFrontierQueue
