...
def test_extract_labeled_procedures_accepts_numbered_product_heading() -> None:
    rows = _extract_labeled_procedures(
        [
            {
                "page_number": 3,
                "text": (
                    ...
                    "Cycloheptanone (2.0 mmol) and cyclopentadiene "
                    "(5.0 mmol) were added to methanol. The reaction mixture "
                    "was stirred and purified to yield the product.\n"
                    "(2) 6,6-dimethylfulvene\n"
                    "Acetone (2.0 mmol) and cyclopentadiene (5.0 mmol) were "
                    "added. The reaction mixture was stirred to yield product."
                ),
            }
        ]
    )
...
def test_native_pdf_name_repairs_are_generic_resolver_candidates() -> None:
    candidates = source_name_resolution_candidates("6,6-HexatnethyIenefulvene7a")

    assert "6,6-Hexamethylenefulvene" in candidates
    assert "cyclopentadiene" in source_name_resolution_candidates(
        "cyclo pentadiene"
    )
...
def test_amount_before_of_reagent_names_are_preserved() -> None:
    names = source_amount_reagent_names(
        "A solution including 5.0 g (18.6 mmol) of "
        "2,7-di-tert-butylfluorene, a hexane solution "
        "(13.5 ml, 21.6 mmol) of n-butyllithium was added. Next, "
        "3.42 g (23.4 mmol) of cyclohexylfulvene was added dropwise."
    )

    assert "2,7-di-tert-butylfluorene" in names
    assert "n-butyllithium" in names
    assert "cyclohexylfulvene" in names


def test_amount_of_name_before_parenthesized_moles_is_preserved() -> None:
    names = source_amount_reagent_names(
        "7 g of cyclohexanone (71 mmol) and 14.35 ml of cyclopentadiene "
        "(175 mmol) were charged in a bottle. Then, 8.75 ml of pyrrolidine "
        "(105 mmol) was added gradually."
    )
...
def test_deterministic_parser_promotes_step_before_exact_downstream_compile(
    ...
    with patch.dict(
        ...
        "cascade_planner.legacy.harness_runtime.tools."
