...
from cascade_planner.legacy.harness_runtime.agentic_blackboard import _pdf_structure_summary
...
def test_visual_execution_reindexes_stale_advisory_page_numbers(
    ...
    with patch(
        "cascade_planner.legacy.harness_runtime.tools.rebuild_literature_pdf_page_focus",
...
def test_pdf_tool_builds_focus_from_host_target_and_source_labels(tmp_path: Path) -> None:
    ...
    with patch(
        "cascade_planner.legacy.harness_runtime.tools.extract_literature_pdf_assets",
        return_value=result,
    ) as extractor:
