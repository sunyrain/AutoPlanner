...
def test_strategy_topology_profile_distinguishes_ring_sizes_from_fusion() -> None:
    ...
    for prompt in (generator_prompt, critic_prompt):
        ...
        assert (
            '"pair_count":1,"ring_sizes":[5,5],"shared_atom_count":0'
            in prompt
        )
        assert (
            '"pair_count":2,"ring_sizes":[5,8],"shared_atom_count":2'
            in prompt
        )
    assert "never infer a fused or spiro relationship from ring_sizes alone" in (
        critic_prompt
    )
...
def test_paper_matched_node_prompt_is_compact_single_reactionjson_policy() -> None:
    ...
    assert "persists across preparatory moves, timeouts" in prompt
    ...
    assert '"schema_version":"paper_matched_route_builder_context.v7"' in prompt
...
def test_critic_prompt_separates_uncertainty_from_reject_and_keeps_full_route() -> None:
    ...
    assert (
        "No Builder checkpoint_relation, role label, or host anchor claim is evidence"
        in prompt
    )
...
def test_v9_strategy_schema_is_query_assumption_and_checkpoint() -> None:
    task = type(
        "PaperStrategyTask",
        (),
        {
...
def test_v9_critic_tasks_use_distinct_provider_contracts() -> None:
    spec = _spec(_context())
    ...
    key_schema = codex_worker_module._chemical_strategy_critique_payload_json_schema(
        key_task
    )
    final_schema = codex_worker_module._chemical_strategy_critique_payload_json_schema(
        final_task
    )
...
def test_v9_key_event_detector_distinguishes_execution_from_deferred_setup() -> None:
    strategy = {
        "strategy_query": (
            "Construct the decalin by an intramolecular Diels-Alder (IMDA)."
        ),
    ...
    assert not sequential_module._step_claims_strategy_key_event(
        {
            "transformation_hypothesis": "claim without a skeletal edit",
            "checkpoint_relation": "executes_checkpoint",
            "reaction_operations": [
...
def test_v9_key_event_prompt_audits_only_focus_step_not_route_completeness() -> None:
    prompt = sequential_module._critic_prompt(
        target="CCO",
        branch_index=0,
        strategy_card={
...
def test_paper_portfolio_preserves_usable_branch_when_one_sidecar_fails(
    ...
    advisory = next(
        row
        for row in plan.route_families
        if row["route_family_id"] == "codex:sequential:family:3"
    )
    ...
    assert result.usage["branch_route_retention"][2][
        "retained_as_replayable_route"
    ] is False
...
def test_sidecar_failure_recovers_deepest_host_replayed_prefix(
    ...
    assert family["paper_policy_budget_failure"]["reason"] == (
        "paper_strategy_sidecar_failed"
    )
    assert "synthetic failure after durable prefix" in family[
        "paper_policy_budget_failure"
    ]["detail"]
    assert result.usage["paper_policy_partial_branch_failure"] is True


def test_rejected_key_event_keeps_second_critic_for_corrected_candidate(
    monkeypatch,
) -> None:
    context = _context()
    config = DirectorConfig(
        ...
        assessment.update(
            {
                ...
                "verdict": verdict,
                "blocking": reject,
                "blocking_type": "mechanism" if reject else "none",
                "reasons": ["synthetic first key-event rejection"] if reject else [],
                "suggested_revision": (
                    "use the corrected graph edit" if reject else ""
                ),
            }
        )
        payload.update(
    ...
    def executor(task):
        ...
        if task.task_type == "paper_matched_key_event_critic":
            reject = not key_critic_verdicts
            key_critic_verdicts.append("reject" if reject else "pass")
            return key_critic_record(task, reject=reject)
        if task.required_artifact_type == "ChemicalStrategyCritique":
            return _critic_record(task)
    ...
    def fake_sidecar(*, request_handler, **_kwargs):
        ...
        second = request_handler(request)["candidates"][0]
        return {
            "route_steps": [second["route_step"]],
            "open_leaf_states": [
                {"smiles": value, "mapped_smiles": mapped}
                for value, mapped in zip(
    ...
    spec = replace(
        ...
        metadata={
            **dict(base_spec.metadata),
            "remaining_model_budget": {"model_invocations": 8},
        },
    )
    result = SequentialStrategyDirectorRunner(
        node_executor=executor,
    ...
    assert key_critic_verdicts == ["reject", "pass"]
    assert result.usage["actual_key_event_critic_calls"] == 2
    plan = GlobalCampaignPlan.from_dict(result.output)
    assert len(plan.multi_step_skeletons[0]["steps"]) == 1
    family = plan.route_families[0]
    assert family["key_event_critic_call_count"] == 2
...
def test_same_aiz_state_reports_no_progress_and_filters_exact_duplicate(
    ...
    assert first_feedback["reason"] == (
        "candidate_did_not_advance_selected_mcts_path"
    )
    assert first_feedback["attempted_net_edits"] == [
        {"map_a": 2, "map_b": 3, "op": "break_bond"}
    ]
    duplicate_feedback = builder_contexts[2]["last_rejection_for_this_leaf"]
    assert duplicate_feedback["reason"] == (
...
def test_key_event_feedback_survives_preparatory_move_and_leaf_change(
        ...
        assessment.update(
            {
                ...
                "reasons": (
                    []
                    if corrected
                    else ["preserve one tethered intramolecular precursor"]
                ),
                "suggested_revision": (
    ...
    assert builder_contexts[1]["pending_checkpoint_feedback"]["reasons"] == [
        expected_reason
    ]
    assert "last_rejection_for_this_leaf" not in builder_contexts[1]
    assert builder_contexts[2]["pending_checkpoint_feedback"]["reasons"] == [
        expected_reason
    ]
...
def test_codex_critic_editor_loop_repairs_a_blocking_step() -> None:
    ...
    builder_tasks = [
        task for task in observed if task.task_type == "route_step_materialization"
    ]
...
def test_paper_critic_reserve_drops_spent_editor_pairs_but_keeps_future_critic() -> None:
    branches = [
    ...
    critics, editors = (
        sequential_module._paper_critic_editor_reserve_after_current_critic(
            branches,
            current_index=0,
            iteration=2,
            max_rounds=6,
...
def test_builder_lineage_uses_host_mapped_sibling_when_aiz_stereo_projection_differs() -> None:
    ...
    context = json.loads(prompt.split("PaperMatchedRouteBuilderContext:\n", 1)[1])
    assert [
        row["reaction_family"] for row in context["connected_path_reactions"]
...
def test_legacy_alias_seed_recovers_input_from_record_provenance(tmp_path) -> None:
    ...
    recovered = SequentialStrategyDirectorRunner(
        ...
        worker_record_seed_path=str(
            alias_dir / "sequential-director-worker-records.jsonl"
        ),
        worker_record_seed_recovery_mode="exact_model_io_v1",
    )
    recovered._prepare_worker_record_journal(fresh_spec)
...
def test_graph_finalization_failure_reaches_builder_with_root_cause() -> None:
    candidate = {
        ...
        "not_parent_route_proof": True,
        "reaction_operations": [
            {"op": "set_explicit_h", "map_idx": 1, "count": 5}
        ],
    }
    diagnostic = _expansion_rejection_diagnostic(
