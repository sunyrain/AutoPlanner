...
- [x] 对固定 provider 请求实现 raw proposal 100% digest parity。
  2026-08-11 使用同一 runtime、请求、vendor、stock、模型内容绑定、环境与 seed 独立执行 embedded/standalone 两臂：路线数均为 2，route fingerprint rows 完全一致，raw proposal digest 均为 `e2b9856279fde1871d029e1a166e410f5268ead92fe53f56d1e1be339dd20e6c`；两臂 operational raw-result digest 因运行回执不同而不同，符合预期。可复用执行器为 `scripts/run_chemenzy_native_parity_probe.py`，证据为 `benchmarks/chemenzy_native_parity_20260811.json`。
...
- [x] 第二十八刀（真实原生双运行一致性证据）：新增 `scripts/run_chemenzy_native_parity_probe.py`，在同一固定请求上独立执行 embedded V4 ingestion 与 standalone launcher，按模型内容完整性、backend failure-free、非空路线集、raw proposal digest 和逐路线 fingerprint 失败关闭判定，并保留两臂原始输出与日志。2026-08-11 实际运行耗时 77.25 秒/70.50 秒，2/2 route、raw proposal digest 与 fingerprint rows 完全一致；不同 operational receipt 仍产生不同 `raw_result_sha256`，没有被误判为科学差异。机器可读证据 `benchmarks/chemenzy_native_parity_20260811.json` 带自校验内容摘要；新增正反例单测覆盖相同 proposal/不同 receipt、backend failure、空 proposal 的拒绝和 proposal 改变的拒绝。现场 CLI 复跑另暴露 fresh/uncached RetroStar stock 的 Windows pandarallel 子进程兼容问题，探针已正确失败关闭并将其保留为 §8.3 未完成项。最终完整离线套件为 `2796 passed, 3 skipped, 11 warnings, 2 subtests passed`，Ruff、compileall、证据内容摘要与 `git diff --check` 均通过。该证据只完成 §8.3 的固定请求门，不外推所有请求的 §8.1 普遍不变量。
