...
- [ ] Windows fresh/uncached RetroStar stock 准备阶段必须让 pandarallel 子进程继承 torchtext legacy compatibility bootstrap；2026-08-11 现场 CLI 复跑在当前未缓存 `origin_dict.csv` 上记录 `backend_initialization_failed`，新探针已按 backend failure-free 与非空路线门返回非零，不会把双方同失败误报为 parity。
