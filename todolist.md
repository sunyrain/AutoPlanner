...
- [x] 对固定 provider 请求实现 raw proposal 100% digest parity。
  2026-08-11 使用同一 runtime、请求、vendor、冻结 SQLite stock（`30c828d6780e534d8368f4eb74f844c889683453080d44053ba298a7bebdd79c`）、模型内容绑定、stock 内容绑定、环境与 seed 独立执行 embedded/standalone 两臂：路线数均为 2，1213 条 expansion trace 摘要完全一致，route fingerprint rows 完全一致，raw proposal digest 均为 `6e1f2f19c5181183df5341afd03ab2af2ad8e7b4298e833d4647ae9fe7ff4418`，provider replay key 为 `65e57b174c906b456310db29449ca643019192a2d3ab8a60709ab88f1c17bbe1`；两臂 operational raw-result digest 因运行回执不同而不同，符合预期。可复用执行器为 `scripts/run_chemenzy_native_parity_probe.py`，证据为 `benchmarks/chemenzy_native_parity_20260811.json`。
