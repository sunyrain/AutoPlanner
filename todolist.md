...
- [ ] 第二十六刀（ChemEnzy seed/replay 保护）：新增显式 `chemenzy_seed`、运行时 RNG 绑定、去除 operational receipt 噪声的 `raw_proposal_sha256`、provider invocation/replay binding，并让 standalone/embedded 比较优先报告 raw proposal digest；capability probe 后续补齐 checkpoint 全文件 SHA-256 与模型数据目录全内容 manifest SHA-256，实际 runtime 的 6 个模型路径内容身份完整且 preflight 为 14.25 秒。相关新增回归已纳入完整离线门；真实固定请求双运行 parity 仍待独立证据，未提前勾选 §8.1/§8.3 的完整验收项。
