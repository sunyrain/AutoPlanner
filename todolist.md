...
- [x] 第二十八刀（真实原生双运行一致性证据）：新增 `scripts/run_chemenzy_native_parity_probe.py`，在同一固定请求上独立执行 embedded V4 ingestion 与 standalone launcher，按模型内容完整性、stock 内容摘要、backend failure-free、非空路线集、search trace、raw proposal digest 和逐路线 fingerprint 失败关闭判定，并保留两臂原始输出与日志。2026-08-11 在提交 `57a5b92` 上使用冻结 SQLite stock 实际运行耗时 79.235 秒/73.906 秒，2/2 route、1213 条 expansion trace、raw proposal digest 与 fingerprint rows 完全一致；不同 operational receipt 仍产生不同 `raw_result_sha256`，没有被误判为科学差异。机器可读证据 `benchmarks/chemenzy_native_parity_20260811.json` 带自校验内容摘要；新增正反例单测覆盖相同 proposal/不同 receipt、backend failure、空 proposal、search trace 改变和 proposal 改变的拒绝。此前未缓存 CSV 路径的现场失败仍保留为运行准备边界，不影响冻结 SQLite 证据。最终完整离线套件为 `2801 passed, 3 skipped, 11 warnings, 2 subtests passed`，Ruff、compileall、证据内容摘要与 `git diff --check` 均通过。该证据只完成 §8.3 的固定请求门，不外推所有请求的 §8.1 普遍不变量。
- [x] 第二十九刀（跨进程 MCTS 确定性）：修复 AutoPlanner-owned bounded MCTS 对 vendor `list(set(MolNode))` 的依赖；成功 sibling 去重与 rollout 并列裁决现在使用稳定 node ID/深度/分子式，不再受独立进程对象地址哈希影响。正反例单测覆盖不同对象 hash、值相同的稳定 tie-break 和深度回退；真实双臂的第 146 行 trace 分歧消失，embedded/standalone 均得到相同 1213 行 expansion trace 和 `6e1f2f19c5181183df5341afd03ab2af2ad8e7b4298e833d4647ae9fe7ff4418`。完整离线套件为 `2801 passed, 3 skipped, 11 warnings, 2 subtests passed`。
