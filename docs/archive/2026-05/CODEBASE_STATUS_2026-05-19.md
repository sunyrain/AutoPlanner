...
`scripts/run_chem_enzy_plan_for_web.py` keeps legacy cascade action/source hooks
off by default. Enabling old action-value/source-policy hooks requires both a
request flag and `AUTOPLANNER_ALLOW_LEGACY_RESEARCH=1`.
...
| Area | Status |
| --- | --- |
| `cascade_planner/cascade_search/` | Active research interface for cascade search, condition state, verifier value hooks, and subgoal proposals. |
| CCTS / runtime rerank / v4 learned-value / provider-retrieval scripts in `cascade_planner/eval/` | Guarded archive. Useful only for reproducing old reports, not the Web default. |
...
| Frozen CCTS/ranker/v4 learned-value/provider/fallback lines | Guarded archive manifest at `archive/code/frozen_research_2026-05-20/README.md`; direct execution requires `AUTOPLANNER_ALLOW_LEGACY_RESEARCH=1`. |
