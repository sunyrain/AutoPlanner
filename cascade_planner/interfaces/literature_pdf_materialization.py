...
from cascade_planner.interfaces.literature_pdf_projection import (
    compile_pdf_replay_assets,
    target_aliases as _target_aliases,
)
...
def finalize_pdf_materialization(
    ...
    *,
    ...
    replay_assets = compile_pdf_replay_assets(
        manifest,
        materialized_dir=materialized_dir,
        max_visual_pages=config.max_visual_pages,
        document_id=document_id,
        source_ref=source_ref,
        pdf_path=pdf_path,
        pdf_sha256=pdf_sha,
    )
    manifest_path = Path(replay_assets["manifest_path"])
    manifest_sha256 = str(replay_assets["manifest_sha256"])
    pages = [dict(row) for row in replay_assets["pages"]]
    source_evidence = [
        dict(row) for row in replay_assets["source_evidence"]
    ]
    target_focus = dict(replay_assets["target_focus"])
