"""Replay validation for immutable biocatalytic Program admission events."""
...
class BiocatalyticAdmissionRecord:
    event: dict[str, Any]
    graph: dict[str, Any]
    route: dict[str, Any]
...
def load_biocatalytic_admission_record(
    path: Path, *, run_id: str, artifacts: ArtifactStore
) -> BiocatalyticAdmissionRecord:
    event = _read_event(path)
    reasons = _validate_event_envelope(event, path=path, run_id=run_id)
    ...
    try:
        reasons.extend(
            _binding_reasons(
                event, graph, route, projection, discovery, bundle, pack
            )
        )
    except (KeyError, TypeError, ValueError) as exc:
        raise BiocatalyticProgramStoreCorruption(
            f"biocatalytic_program_artifact_binding_failed:{type(exc).__name__}"
        ) from exc
    reasons.extend(_ref_reasons(event, refs))
    validation_rows = list(pack.get("validations") or [])
    ...
    if reasons:
        raise BiocatalyticProgramStoreCorruption(
            "biocatalytic_program_event_invalid:" + ",".join(sorted(set(reasons)))
        )
    return BiocatalyticAdmissionRecord(
        event=event,
        graph=graph,
        route=route,
...
def _read_event(path: Path) -> dict[str, Any]:
    try:
        ...
        raise BiocatalyticProgramStoreCorruption(
            f"biocatalytic_program_event_unreadable:{type(exc).__name__}"
        ) from exc
    if not isinstance(value, dict):
        raise BiocatalyticProgramStoreCorruption("biocatalytic_program_event_not_object")
    return dict(value)
...
def _validate_event_envelope(
    ...
    if event.get("schema_version") != BIOCATALYTIC_PROGRAM_ADMISSION_EVENT_SCHEMA:
        reasons.append("event_schema_invalid")
    if event.get("run_id") != run_id:
        reasons.append("event_run_id_mismatch")
    if event.get("admission_policy") != BIOCATALYTIC_PROGRAM_ADMISSION_POLICY:
    ...
    if event.get("event_identity_sha256") != identity or event.get("event_id") != (
        f"biocatalytic-program-admission:sha256:{identity}"
    ):
...
def _read_payloads(
    ...
    try:
        ...
        raise BiocatalyticProgramStoreCorruption(
            f"biocatalytic_program_artifact_replay_failed:{type(exc).__name__}"
        ) from exc
...
def _binding_reasons(
    ...
    if (
        ...
        or pack.get("schema_version") != BIOCATALYSIS_VALIDATION_PACK_SCHEMA
...
__all__ = ["BiocatalyticAdmissionRecord", "load_biocatalytic_admission_record"]
