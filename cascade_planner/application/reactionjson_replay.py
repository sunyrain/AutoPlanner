...
REACTIONJSON_PROFILE = "reactionjson_public_profile.2026-08-12.v0"
...
_FIELDS = {
    ...
    "set_bond_stereo": _COMMON | {"map_a", "map_b", "stereo", "stereo_atom_maps"},
}
_BOND_TYPES = {
...
def _apply(molecule: Chem.RWMol, row: Mapping[str, Any]) -> Chem.RWMol:
    kind = str(row["op"])
    if kind == "break_bond":
...
def _change_atom(molecule: Chem.RWMol, row: Mapping[str, Any]) -> None:
    ...
    if "formal_charge" in row:
        atom.SetFormalCharge(
            _integer(row["formal_charge"], "reactionjson_charge_invalid")
        )
    if "isotope" in row:
        atom.SetIsotope(
            _integer(row["isotope"], "reactionjson_isotope_invalid", minimum=0)
        )
...
def _add_group(molecule: Chem.RWMol, row: Mapping[str, Any]) -> Chem.RWMol:
    ...
    combined.AddBond(
        anchor_index,
        offset + neighbor.GetIdx(),
        attachment_bond.GetBondType(),
    )
...
def _set_bond_stereo(molecule: Chem.RWMol, row: Mapping[str, Any]) -> None:
    ...
    if stereo_name not in {"NONE", "ANY"}:
        ...
        stereo_indices = [_map_index(molecule, value) for value in stereo_maps]
        bond.SetStereoAtoms(*stereo_indices)
