...
def exact_edge_signature(product_smiles: Any, precursor_smiles: Iterable[Any]) -> str:
    ...
    The graph-v1 display signature joins components with ``.`` and is
    ambiguous when one precursor is itself multicomponent.  The ledger hashes
    a typed JSON tuple instead, while retaining the source graph signature as
    non-authoritative provenance on the edge row.
    """
