"""Adapt whole-cell and hybrid Program drafts into the common route space."""
...
def compile_execution_program_route_candidates(
    ...
    *,
    ...
    proposals = dict(execution_bundle.get("program_proposals") or {})
    candidates: dict[str, dict[str, Any]] = {}
    for route_candidate_id, raw_variant in sorted(
        dict(execution_bundle.get("route_candidates") or {}).items()
    ):
        variant = dict(raw_variant)
        proposal_id = str(variant.get("execution_program_id") or "")
        proposal = dict(proposals.get(proposal_id) or {})
        if not proposal:
            raise ProgramRouteCandidateError(
                f"program_candidate_execution_missing:{route_candidate_id}"
            )
        candidate = _candidate(
            ...
            source_bundle_sha256=str(execution_bundle.get("content_sha256") or ""),
        )
...
def _candidate(
    ...
    *,
    ...
    domain = str(proposal.get("execution_domain") or "")
    validation_gate = dict(proposal.get("validation_plan") or {})
    validated = validation_gate.get("accepted") is True
    if (
        domain not in {"whole_cell", "hybrid"}
        or variant.get("full_candidate_route_restitched") is not True
        or not selected
        ...
        raise ProgramRouteCandidateError(
            f"program_candidate_execution_mapping_invalid:{route_candidate_id}"
        )
    ledger = dict(proposal.get("cofactor_and_carrier_ledger") or {})
    cofactor_systems = len(
        {
            *dict(ledger.get("cofactor_requirements") or {}),
            *dict(ledger.get("carrier_requirements") or {}),
        }
    )
    metrics = canonical_route_metrics(
        route,
        physical=int(variant.get("physical_operation_count") or 0),
        ...
        cofactor_systems=cofactor_systems,
    )
    ...
    if validated:
        ...
        metrics["condition_deficit_count"] = max(
            1, int(metrics["condition_deficit_count"])
        )
    metrics["process_deficit_count"] = max(1, int(metrics["process_deficit_count"]))
    metrics["source_deficit_count"] = max(1, int(metrics["source_deficit_count"]))
    metrics["risk_data_deficit_count"] = max(
        1, int(metrics["risk_data_deficit_count"])
    )
    claim_refs = dict(proposal.get("claim_refs") or {})
    authority_snapshot = {
        "canonical_route": canonical_route_authority_snapshot(route),
        "execution_program_id": proposal_id,
        "execution_domain": domain,
        "source_capability_sha256": str(
            proposal.get("source_capability_sha256") or ""
        ),
        "replaced_edge_ids": [
            str(value) for value in variant.get("replaced_edge_ids") or []
        ],
        "full_candidate_route_restitched": True,
    }
    return build_program_route_candidate(
        candidate_id=(
            f"program-route:{domain}:"
        ...
        specialized_validation_ids=[
            str(value)
            for value in validation_gate.get("accepted_validation_ids") or []
        ],
        source_refs=normalize_strings(claim_refs.get("precedent_refs")),
        ...
        warning_codes=sorted(
            {
                ...
                *(
                    ["EXECUTION_VALIDATION_BOUND"]
                    if validated
                    else ["SPECIALIZED_EXECUTION_VALIDATION_REQUIRED"]
                ),
            }
        ),
...
__all__ = ["compile_execution_program_route_candidates"]
