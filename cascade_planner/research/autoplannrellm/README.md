...
The integration is opt-in through the research runner. The active route-tree
runtime does not inspect these variables or import this package:
...
```bash
python -m cascade_planner.research.autoplannrellm.runner -- \
...
The runner starts
`cascade_planner.research.autoplannrellm.live_benchmark` as the parallel worker.
That worker injects the DeepSeek controller and optional candidate appender via
`RouteTreeExtensions`; the canonical benchmark worker remains research-free.
...
Prior-provider comparison is research-only:
