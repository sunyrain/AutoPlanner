...
from cascade_planner.legacy.eval_runtime.train_route_pool_ranker import _float
...
def rerank_cascade_only_features_with_product_audit(
    *,
    ...
    result = {
        ...
        "native_audit_summary": summarize_product_audit(audit),
        "guarded_audit_summary": summarize_product_audit(reranked_audit),
        "native_ranked_product_metrics": ranked_product_metrics(audit),
        "guarded_ranked_product_metrics": ranked_product_metrics(reranked_audit),
        "native_gt_recovery": ground_truth_recovery(run, benchmark_rows),
        "guarded_gt_recovery": ground_truth_recovery(output_run, benchmark_rows),
        "delta_guarded_minus_native": product_audit_delta(audit, reranked_audit),
...
def _load_run(path: Path, *, top_k: int | None) -> dict[str, Any]:
    ...
    for target in run.get("targets") or []:
        payload = dict(target)
        routes = routes_for_target(target)[: int(top_k)]
...
def _run_summary(targets: list[dict[str, Any]]) -> dict[str, Any]:
    route_counts = [len(routes_for_target(target)) for target in targets]
    return {
        "targets": len(targets),
...
if __name__ == "__main__":
    ...
    require_legacy_research_enabled("legacy cascade-only product-audit reranker")
    main()
