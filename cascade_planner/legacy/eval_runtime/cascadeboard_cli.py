...
Usage:
  python -m cascade_planner.legacy.eval_runtime.cascadeboard_cli --target "CC(=O)O" --n-steps 2
  python -m cascade_planner.legacy.eval_runtime.cascadeboard_cli --target "CC(=O)O" --constraints '{"one_pot": true, "max_delta_T": 15}'
  python -m cascade_planner.legacy.eval_runtime.cascadeboard_cli --target "CC(=O)O" --objective green --checkpoint results/shared/cascadeboard_model_v13/best.pt
"""
...
def main():
    ...
    # Build planner
    from cascade_planner.legacy.eval_runtime.cascadeboard_planner import (
        CascadeBoardPlanner,
...
if __name__ == "__main__":
    ...
    require_legacy_research_enabled("legacy CascadeBoard standalone CLI")
    main()
