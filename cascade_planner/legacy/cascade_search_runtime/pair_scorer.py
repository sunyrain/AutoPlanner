...
def pair_feature_vector(row: dict[str, Any], schema: dict[str, Any]) -> np.ndarray:
    ...
    for field_name in schema.get("categorical_fields") or []:
        value = _feature_field(row, field_name)
        categories = (schema.get("categories") or {}).get(field_name) or []
        out.extend(1.0 if value == category else 0.0 for category in categories)
    features = pair_rule_features(row)
    out.extend(
        float(features.get(field_name, 0.0))
        for field_name in schema.get("numeric_fields") or []
    )
