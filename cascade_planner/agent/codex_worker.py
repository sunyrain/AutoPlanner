...
def _paper_matched_route_step_contract_reasons(payload: Any) -> list[str]:
    ...
    if len(candidates) != 1:
        ...
        if str(candidate.get("step_role") or "") not in {
            "key",
            "enabling",
            "supporting",
        }:
            reasons.append("paper_route_step_move_role_invalid")
    ...
    *,
    ...
    if task.task_type == "paper_matched_strategy_generator":
        ...
        candidate = {
            ...
            "step_role": str(result.get("move_role") or ""),
...
def _strategy_portfolio_report_payload_json_schema(task: WorkerTask) -> dict[str, Any]:
    paper_matched = task.task_type == "paper_matched_strategy_generator"
